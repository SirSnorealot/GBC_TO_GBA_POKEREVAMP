...
| **Output** | *Save to output/* writes `output\<name>\revamped.png` (the current frame), `revamped_shiny.png`, `compare.png`, `report.json` and `session.json`; when more than one frame is selected also `revamped_frames.png` and `revamped_shiny_frames.png` (the chosen frames stacked vertically). *Save / Load session* stores every setting, the color map, the frame selection and all paint edits for every frame. |
